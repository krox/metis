...
constexpr int polyglot_piece_index(Piece p)
{
	// Polyglot piece order:
	// 0 black pawn, 1 white pawn, 2 black knight, 3 white knight,
	// 4 black bishop, 5 white bishop, 6 black rook, 7 white rook,
	// 8 black queen, 9 white queen, 10 black king, 11 white king.
	switch (p)
	{
...
uint64_t castling_component(CastlingRights rights)
{
	uint64_t key = 0;
...
uint64_t ep_component(Bitboard ep_square)
{
	if (!any(ep_square))
		return 0;
	int ep_file = first_set(ep_square) % 8;
	return polyglot_random[772 + ep_file];
}

bool ep_capture_possible(std::array<Piece, 64> const &squares,
                         Bitboard ep_square, Color side_to_move)
{
	if (!any(ep_square))
		return false;

	int ep_sq = first_set(ep_square);
	int file = ep_sq % 8;
	int rank = ep_sq / 8;
	int pawn_rank = side_to_move == Color::White ? rank - 1 : rank + 1;
	if (pawn_rank < 0 || pawn_rank > 7)
		return false;

	auto pawn = make_piece(PieceType::Pawn, side_to_move);
	for (int df : {-1, 1})
	{
		int f = file + df;
		if (f < 0 || f > 7)
			continue;
		int sq = pawn_rank * 8 + f;
		if (squares[sq] == pawn)
			return true;
	}
...
// Polyglot random numbers (standard).
...
void Board::place_piece(Piece p, int sq)
{
	...
	auto idx = polyglot_piece_index(p);
	zobrist_key_ ^= polyglot_random[64 * idx + sq];
}
...
void Board::recompute_zobrist()
{
	...
	zobrist_key_ ^= castling_component(castling_rights);
	zobrist_key_ ^= ep_component(ep_square);
	if (color_to_move == Color::White)
		zobrist_key_ ^= polyglot_random[780];
}
...
Board Board::from_fen(util::Parser &parser)
{
	...
	util::check(ranks.size() == 8, "invalid FEN string: expected 8 ranks");
	for (size_t rank = 0; rank < 8; ++rank)
	{
		...
		for (auto c : ranks[rank])
		{
			...
			{
				util::check(file < 8,
				            "invalid FEN string: too many pieces on rank");
				b.place_piece(char_to_piece(c), (7 - rank) * 8 + file);
				++file;
			}
		}
		util::check(file == 8, "invalid FEN string: not enough pieces on rank");
	}

	// parts[1]: who's turn it is
	auto side = parser.word();
	util::check(side == "w" || side == "b",
	            "invalid FEN string: expected 'w' or 'b' for side to move");
	...
	if (!parser.match("-"))
		for (char c : parser.word())
		{
			if (c == 'K')
				...
				util::check(
				    false,
				    "invalid FEN string: invalid castling rights character");
		}

	// parts[3]: en passant square
	if (auto ep = parser.word(); ep != "-")
	{
		util::check(ep.size() == 2 && 'a' <= ep[0] && ep[0] <= 'h' &&
		                '1' <= ep[1] && ep[1] <= '8',
		            "invalid FEN string: invalid en passant square");
		b.ep_square = square((ep[0] - 'a') + 8 * (ep[1] - '1'));
	}

	// important convention: only set ep_square if an en passant capture is
	// actually possible.
	if (!ep_capture_possible(b.squares_, b.ep_square, b.color_to_move))
		b.ep_square = Bitboard::none;

	// skip optional halfmove/fullmove numbers (TODO...)
	parser.integer();
	parser.integer();

	util::check(b.valid(), "invalid FEN string: invalid board position");
	b.compute_attacks();
	b.recompute_zobrist();
	return b;
}

GameState GameState::from_uci(std::string_view uci)
{
	auto parser = util::Parser(uci);
	return from_uci(parser);
}

GameState GameState::from_uci(util::Parser &parser)
{
	auto state = GameState(Board::from_fen(parser));
	if (parser.ident("moves"))
		while (!parser.end())
			state.push_move(Move(parser.expect_ident()));
	parser.expect_end();
	return state;
}
...
void Board::make_move(Move move)
{
	...
	// remove old zobrist components (to be added back later)
	zobrist_key_ ^= castling_component(castling_rights);
	zobrist_key_ ^= ep_component(ep_square);
	...
	if (pawn_double_push)
	{
		auto candidate_ep = square((move.from + move.to) / 2);
		if (ep_capture_possible(squares_, candidate_ep, color_to_move))
			ep_square = candidate_ep;
		zobrist_key_ ^= ep_component(ep_square);
	}

	zobrist_key_ ^= polyglot_random[780]; // side-to-move component
	compute_attacks();

#ifdef METIS_DEBUG_ZOBRIST
	auto expected_zobrist = zobrist_key_;
	recompute_zobrist();
	assert(zobrist_key_ == expected_zobrist &&
